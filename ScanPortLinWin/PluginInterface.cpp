...
#ifdef _WIN32
	pluginFiles.push_back(pluginsDir + "PluginScanerUDP.dll");
	pluginFiles.push_back(pluginsDir + "PluginScanerTCP.dll");
#else
	pluginFiles.push_back(pluginsDir + "UDP.so");
	pluginFiles.push_back(pluginsDir + "TCP.so");
#endif
...
void loaderPlugins::executePlugin(int s, int e) {
	for (int i = 0; i < this->Plugin.size(); i += 1) {
		this->Plugin[i]->InitPlugin();
		this->Plugin[i]->WorkPlugin(s,e);
	}
}
